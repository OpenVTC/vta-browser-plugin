...
| **`@openvtc/pnm-core/admin`** | Agent administration: `acl/*`, `keys/*`, `policy/*`, `device/*` (list, disable, wipe), `vta/did-templates/*`, `vta/memory/*`, `audit/list`, `config/{show,patch}`, session introspection, and context deletion. Operator surface — not in the root barrel, import it explicitly. |
